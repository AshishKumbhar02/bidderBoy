                        ...
                        @foreach ($credits as $row)
                            <tr>
                                ...
                                <td>
                                    @if ($row->status == 1)
                                        <a
                                            href="{{ route('customers.update_credit_request', ['id' => $row->id, 'status' => '0', 'credit' => '-' . $row->credits, 'user_id' => $row->user_id]) }}">Disapprove</a>
                                    @else
                                        <a
                                            href="{{ route('customers.update_credit_request', ['id' => $row->id, 'status' => '1', 'credit' => $row->credits, 'user_id' => $row->user_id]) }}">Approve</a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
