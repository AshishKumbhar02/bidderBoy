            ...
            <div class="card-header p-3">
                <form action="" method="get" class="">
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                <select name="is_active" class="form-control">
                                    ...
                                    <option value="inactive" @if ($param['is_active'] == 'inactive') selected @endif>Inactive
                                    </option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-md-1">
                            <div class="form-group">
                                <a href="{{ url('admin/customers') }}" class="btn btn-primary btn-block"><i
                                        style="font-size: 21px;" class="fa-solid fa-refresh"></i></a>
                            </div>
                        </div>
                        <div class="col-md-1">
                            <div class="form-group">
                                <a href="{{ route('export.allCustomers', request()->input()) }}"
...
@else
    ...
    @endif
                                        ...
                                        <td>
                                            <a href="{{ route('customers.edit_customer', $row->id) }}"
                                                class="btn btn-success"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                            @if ($row->kyc_document_verification == 1 && !empty($row->kyc_document))
                                                <a class="btn btn-warning"
                                                    href="{{ route('customers.update_kyc_ducument_status', ['id' => $row->id, 'status' => '0']) }}"
                                                    title="Disapprove KYC"><i class="fa fa-key"
                                                        aria-hidden="true"></i></a>
                                            @elseif($row->kyc_document_verification == 0 && !empty($row->kyc_document))
                                                <a class="btn btn-primary"
                                                    href="{{ route('customers.update_kyc_ducument_status', ['id' => $row->id, 'status' => '1']) }}"
                                                    title="Approve KYC"><i class="fa fa-key" aria-hidden="true"></i></a>
                                            @endif
                                            @if ($row->is_active == 1)
                                                <a style="background: red; border: none;" class="btn btn-primary"
                                                    href="{{ route('customers.update_customer_status', ['id' => $row->id, 'status' => '0']) }}"
                                                    title="Deactivate"><i class="fa fa-lock" aria-hidden="true"></i></a>
                                            @else
                                                <a style="background: blue; border: none;" class="btn btn-primary"
                                                    href="{{ route('customers.update_customer_status', ['id' => $row->id, 'status' => '1']) }}"
                                                    title="Activate"><i class="fa fa-unlock" aria-hidden="true"></i></a>
                                            @endif

                                            <a style="background: red; border: none;" class="btn btn-primary"
                                                href="{{ route('customers.delete', $row->id) }}"
