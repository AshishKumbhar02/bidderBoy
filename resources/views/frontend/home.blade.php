...
<main id="main">
      ...
      <div class="container">
         <div class="row" data-aos="zoom-in">
            <div class="col-sm-2 col-md-6">
               <button type="button" class="btn auction_button active"> <img src="{{url('/assets/front')}}/img/boy-footer.png">Live Auctions</button>
            ...
            <div class="col-md-4 col-sm-6" id="product-{{$row->id}}">
               <div class="product-grid pro_mrg_1 product-active">
                  ...
                  <div class="product-content">
                     ...
                     <h6 class="bidder-name-{{$row->id}} name_of_bidder name_of_bidder_{{$row->id}}">
                        @php

                        echo (!empty($row->last_bidder_name)) ? $row->last_bidder_name:"No New Bidders";


                        @endphp </h6>
                     <h3 class="title"><a href="{{url('auction/'.$row->id)}}">Auction ID: {{$row->id}}</a></h3>
                     <h4 class="title_hed"><a href="{{url('auction/'.$row->id)}}" data-seconds="{{$time_left}}" data-product-id="{{$row->id}}" class="product_timer_div product_timer_{{$row->id}} ">
                     ...
                     <div class="product_bottom_bx">
                        ...
                        @if(Auth::check())
                        <a type="button" class="auto_bid_product" data-bs-toggle="modal" data-bs-target="#myModal" data-product-name="{{$row->product_title}}" data-id="{{$row->id}}"><img class="cardbtn" src="{{url('assets/front')}}/img/auto-button.png"></a>

                        <button type="button" class="btn btn-info mrgleft10 bid_now" data-id="{{$row->id}}" data-amount="{{$row->credit_per_bid}}" data-user="{{Auth::user()->id}}" data-token="{{csrf_token()}}">BID NOW</button>
                        @else
                        <a href="{{url('login')}}" data-product-name="{{$row->product_title}}" data-id="{{$row->id}}"><img class="cardbtn" src="{{url('assets/front')}}/img/auto-button.png"></a>
               ...
               <div style="background: white;" class="modal-dialog">
                  <div class="modal-content">
                     ...
                     <div class="modal-body">
                        ...
                        <input name="txtMaxBidCredit" type="number" id="MaxBidCredit" placeholder="i.e 5 or 20 or 100" class="form-control">
                        Activate at Rs *
                        <input name="txtStartRs" type="number" id="StartRs" placeholder="i.e 2.30 or 0.10" class="form-control">
   ...
   <section class="upcoming_auction">
      <div class="container">
         <div class="button_hed_auction text-center">
            <button type="button" class="btn auction_button1 mrgleft105"> <img src="{{url('assets/front')}}/img/boy-footer.png">Upcoming Auctions</button>
         </div>
   ...
   <section class="testiminials_sections">
      <div class="container">
         <div class="button_hed_auction text-center">
            <button type="button" class="btn auction_button1 mrgleft80"> <img src="{{url('assets/front')}}/img/boy-footer.png">Testimonials</button>
         </div>
         <div class="row">
   ...
   <script>
      $(document).ready(function() {
         ...
         $('.product_timer_div').each(function() {
            var $timerDiv = $(this);
            var timer_string = $timerDiv.text().trim(); // Get the timer string from the element's content
            var timer_parts = timer_string.split(':'); // Split the string into parts: hours, minutes, seconds
            ...
            $.ajax({
               ...
               success: function(data) {
                  ...
                  if (data.status == 'success') {
                     /* $('.name_of_bidder_'+product_id).text(data.product.last_bid_user_name); */
                     /* $('.product_cur_price'+product_id).text('Rs. '+data.product.amount_per_bid); */
                     console.log(data.data.amount_per_bid, data.data.product_id, data.data.name);
                     let message = [data.data.amount_per_bid, data.data.product_id, data.data.name];
